...
> **一个本科毕业设计的梦想，等了三年，终于被 AI 圆满了。**

---

## 📖 写在前面

2022 年冬天，我在做本科毕业设计。课题是 YOLO 目标检测——那时候 YOLOv8 刚发布，我窝在实验室里，对着黑漆漆的终端敲 `python train.py`，等上几个小时，看一串数字跳出来告诉我 mAP 提高了 0.3 个百分点。

说实话，那时候我心里一直有个念头：**为什么这些东西非要用命令行呢？** 为什么不能像 Photoshop 一样，左边选工具、右边调参数、中间是画布，点一下就能看到检测结果？为什么训练一个模型要手动改 YAML 文件里的参数，而不是在网页上点几个按钮就搞定？
...
**然后 2025 年来了。Claude Code 来了。**

坐在电脑前，我打开这个尘封已久的项目文件夹。这一次我不再是一个人——我有了一个几乎无所不能的 AI 搭档。我说「我想要一个检测工作室，左边是工具栏，中间是画布，右边是参数面板」，它就开始写代码。我说「这里面的参数要完全对应 YOLO 的 default.yaml」，它就读了那个文件，把 68 个参数一个一个地搬到了网页上。我说「终端输出要实时显示在页面上」，它就打通了后端和前端。

几百轮对话下来，我本科毕业设计里写下的每一个「未来展望」，都变成了可以点击的按钮、可以拖动的滑块、可以实时查看的画面。**三年前的梦想，被 AI 圆满了。**

这个项目既是我对计算机视觉的热爱，也是一个时代的见证——AI 不是来取代我们的，是来帮我们实现那些曾经「做不到」的事情的。

希望你喜欢这个平台。也希望你也有一个被 AI 圆满的梦想。

---

> 本项目最初是本科毕业设计，在 **Claude Code**（Anthropic 的 AI 编程助手）的辅助下，从命令行脚本升级为了一个**功能完整的 Web 视觉 AI 平台**——支持实时摄像头检测、多任务切换、完整参数训练控制台、模型管理、检测历史记录等功能。
...
| 2025 年夏 | 💡 重新打开项目。这一次，有了 Claude Code |
| Day 1-3 | 🏗️ 搭建 Flask 后端 + YOLO API 封装，跑通第一个实时推理 |
| Day 4-7 | 🎨 前端从零开始——首页、侧边栏、检测工作室的三栏布局 |
| Day 8-14 | 📹 实时摄像头检测 + 图片上传推理，解决坐标映射、Canvas 渲染 |
| Day 15-20 | ⚡ 训练控制台 + 测试控制台，68 个参数逐一从 default.yaml 迁移 |
| Day 21-25 | 🔄 多任务切换、检测历史存储、Photoshop 风格折叠面板 |
| Day 26-28 | 🎨 三主题切换、文件路径选择器、API 全面测试 |
| Day 29 | 📖 写 README，回忆三年前的那个冬天 |
| 2025.07.20 | 🚀 GitHub 开源发布——梦想成真 |
...
> *"三年前写在毕业设计未来展望里的梦想，被 AI 一行一行地写成了现实。"*
